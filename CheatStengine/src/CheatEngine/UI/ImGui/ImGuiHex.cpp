...
static size_t CalcBytesPerLine(float bytes_avail_x, const ImVec2& byte_size, const ImVec2& spacing, bool show_ascii, const ImVec2& char_size, size_t separators)
{
    const float byte_width = byte_size.x + spacing.x + (show_ascii ? char_size.x : 0.f);
    size_t bytes_per_line = (size_t)(bytes_avail_x / byte_width);
    bytes_per_line = bytes_per_line <= 0 ? 1 : bytes_per_line;

    size_t actual_separators = separators > 0 ? (size_t)(bytes_per_line / separators) : 0;
...
#endif
    const size_t c = l > 0.5f ? 0 : 255;
    return IM_COL32(c, c, c, 255);
}

static bool RangeRangeIntersection(size_t a_min, size_t a_max, size_t b_min, size_t b_max, size_t* out_min, size_t* out_max)
{
...
static void RenderByteDecorations(ImDrawList* draw_list, const ImRect& bb, ImColor bg_color,
    ...
    size_t offset, size_t range_min, size_t range_max, size_t bytes_per_line, size_t i, size_t line_base)
{
    ...
    const size_t start_line = range_min / bytes_per_line;
    const size_t end_line = range_max / bytes_per_line;
    const size_t current_line = line_base / bytes_per_line;
...
bool ImGui::BeginHexEditor(const char* str_id, ImGuiHexEditorState* state, const ImVec2& size, ImGuiChildFlags child_flags, ImGuiWindowFlags window_flags)
{
    ...
    size_t address_max_chars;
    if (state->ShowAddress) {
        size_t address_chars = state->AddressChars;
        if (address_chars == -1)
    ...
    size_t actual_separators = (size_t)(bytes_per_line / state->Separators);
    ...
    const size_t select_start_subbyte = state->SelectStartSubByte;
    const size_t select_end_byte = state->SelectEndByte;
    const size_t select_end_subbyte = state->SelectEndSubByte;
    const size_t last_selected_byte = state->LastSelectedByte;
    const size_t select_drag_byte = state->SelectDragByte;
    const size_t select_drag_subbyte = state->SelectDragSubByte;

    size_t next_select_start_byte = select_start_byte;
    size_t next_select_start_subbyte = select_start_subbyte;
    size_t next_select_end_byte = select_end_byte;
    size_t next_select_end_subbyte = select_end_subbyte;
    size_t next_last_selected_byte = last_selected_byte;
    size_t next_select_drag_byte = select_drag_byte;
    size_t next_select_drag_subbyte = select_drag_subbyte;
    ...
    if (state->EnableClipboard && ImGui::IsKeyChordPressed(ImGuiMod_Ctrl | ImGuiKey_C)) {
        if (state->SelectStartByte != -1) {
            const size_t bytes_count = (state->SelectEndByte + 1) - state->SelectStartByte;
            ...
            if (bytes) {
                ...
                if (read_bytes > 0) {
                    ...
                    const size_t text_byte_size = 3;

                    ImGui::LogToClipboard();

                    for (size_t i = 0, abs_i = state->SelectStartByte; i < bytes_count; i++, abs_i++) {
        ...
        for (ImGuiKey key = ImGuiKey_A; key != ImGuiKey_G; key = (ImGuiKey)((size_t)key + 1)) {
        ...
        if (hex_key_pressed == ImGuiKey_None) {
            for (ImGuiKey key = ImGuiKey_0; key != ImGuiKey_A; key = (ImGuiKey)((size_t)key + 1)) {
                if (ImGui::IsKeyPressed(key)) {
                    hex_key_pressed = key;
                    break;
                }
    ...
    while (clipper.Step()) {
        const size_t clipper_lines = clipper.DisplayEnd - clipper.DisplayStart;
        ...
        {
            size_t count = clipper_lines * bytes_per_line * 2;
            if (show_ascii)
        ...
        for (size_t n = clipper.DisplayStart; n != clipper.DisplayEnd; n++) {
            ...
            for (size_t i = 0; i != bytes_per_line; i++) {
                ...
                const size_t offset = bytes_per_line * n + i;
                unsigned char byte;
                ...
                if (offset >= select_start_byte && offset <= select_end_byte) {
                    ...
                    if (!single_highlight) {
                        for (size_t j = 0; j != state->HighlightRanges.Size; j++) {
                ...
                if (offset == last_selected_byte && !state->ReadOnly && hex_key_pressed != ImGuiKey_None) {
                    ...
                    const size_t subbyte = offset == select_start_byte ? select_start_subbyte : select_end_subbyte;
                    ...
                    size_t* next_subbyte = (size_t*)(offset == select_start_byte ? &next_select_start_subbyte : &next_select_end_subbyte);
                    if (!subbyte) {
                        next_select_start_byte = offset;
                        next_select_end_byte = offset;
                        *next_subbyte = 1;
...
bool ImGui::CalcHexEditorRowRange(size_t row_offset, size_t row_bytes_count, size_t range_min, size_t range_max, size_t* out_min, size_t* out_max)
{
    size_t abs_min;
    size_t abs_max;
